...
material_id_t add_material(vec4_t color, float diffuse, texture_id_t texture)
{
    ...
    material_s material = {.diffuse = diffuse, .diffuse_texture = texture};
...
#define max_len                                                                                    \
    256  // a line cannot exceed this or it will be cut. for plain obj this is safe except in
        ...
        while (fgets(buffer, max_len, f))
        {
            ...
            {  // if a face
                GLuint p[3];
                GLuint t[3];
                sscanf_s(buffer, "f %d/%d/%*d %d/%d/%*d %d/%d/%*d", &p[0], &t[0], &p[1], &t[1],
                         &p[2], &t[2]);
